...
async function getCommitteeHeroContent(lang: string) {
  try {
...
const FALLBACK = {
  ...
  title: 'Committees Supporting The Board',
  subtitle: 'Expert committees dedicated to governance and risk management',
  ctaText: "Let's Talk",
  ctaLink: '#',
...
export default async function CommitteeHeroSectionWrapper({ lang }: { lang: string }) {
  const hero = await getCommitteeHeroContent(lang) ?? FALLBACK;
