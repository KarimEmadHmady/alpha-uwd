...
export const UserService = {
  ...
  async login(email, password) {
    ...
    // Use JWT_SECRET from env, fallback to "cinbon" for backward compatibility
    const jwtSecret = process.env.JWT_SECRET || "cinbon";
    const token = jwt.sign({ id: user.id, role: user.role }, jwtSecret, { expiresIn: "10d" });
    return { user, token };
  },
  ...
  async forgotPassword(email) {
    ...
    const resetUrl = `http://localhost:3000/en/login/reset-password?token=${resetToken}&email=${email}`;

    const transporter = nodemailer.createTransport({
      host: "mail.uwd.agency", // ✅from the cPanel
      port: 465, // ✅ SSL/TLS
      ...
      tls: {
        rejectUnauthorized: false // ✅ important if u work  Local
      }
    });
    
    await transporter.sendMail({
      to: email,
      from: `"UWD Support" <${process.env.EMAIL_USER}>`, // ✅ name + email
