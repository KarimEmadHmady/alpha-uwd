...
router.put("/:id", auth, authController.restrictTo('admin', 'manager'), upload.fields([
  { name: "image", maxCount: 1 },
  { name: "fund_manager_image", maxCount: 1 },
]), async (req, res) => {
  try {
...
router.put("/status/:id", auth, authController.restrictTo('admin'), async (req, res) => {
  try {
    // تأكد أن status رقم
...
router.put("/status/decline/:id", auth, authController.restrictTo('admin'), async (req, res) => {
  try {
